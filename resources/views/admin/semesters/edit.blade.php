...
@section('page_subtitle', 'Modify semester details')
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <i class="bi bi-pencil"></i> Edit Semester
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.semesters.update', $semester) }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="course_id" id="course_id" class="form-control @error('course_id') is-invalid @enderror" required>
                                ...
                                @foreach ($courses as $course)
                                    <option value="{{ $course->id }}" {{ old('course_id', $semester->course_id) == $course->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="number" name="number" id="number" class="form-control @error('number') is-invalid @enderror"
                                value="{{ old('number', $semester->number) }}" placeholder="e.g., 1, 2, 3, 4" min="1" required>
                            @error('number')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="label" id="label" class="form-control @error('label') is-invalid @enderror"
                                value="{{ old('label', $semester->label) }}" placeholder="e.g., First Year, Second Year" required>
                            @error('label')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
