...
@section('page_subtitle', 'Modify course details')
                    ...
                    <form action="{{ route('admin.courses.update', $course) }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="university_id" id="university_id" class="form-control @error('university_id') is-invalid @enderror" required>
                                ...
                                @foreach ($universities as $university)
                                    <option value="{{ $university->id }}" {{ old('university_id', $course->university_id) == $university->id ? 'selected' : '' }}>
                                        {{ $university->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="type" id="type" class="form-control @error('type') is-invalid @enderror"
                                value="{{ old('type', $course->type) }}" placeholder="e.g., Bachelor, Master" required>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="language" id="language" class="form-control @error('language') is-invalid @enderror" required>
                                ...
                                <option value="gujarati" {{ old('language', $course->language) == 'gujarati' ? 'selected' : '' }}>
                                    Gujarati
                                </option>
                            </select>
                            @error('language')
